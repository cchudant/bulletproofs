...
pub struct BulletproofGens {
    ...
    // TODO(merge): the use of Arc is to make derive(Clone) happy, as `VartimeRistrettoPrecomputation` is not `Clone`.
    //              I don't know if we want to make `VartimeRistrettoPrecomputation` Clone
    precompute: Option<Arc<VartimeRistrettoPrecomputation>>,
}
...
impl BulletproofGens {
    ...
    ///
    /// This will discard the table generated by [`precompute_table`] if present and
    /// will not regenerate it.
    ...
    /// Compute the precompute tables for multi-base multi-scalar multiplication. By default, `BulletproofGens` will not use any.
    /// This may take a big amount of memory when `gens_capacity * party_capacity` is large, use wisely.
    pub fn precompute_tables(&mut self) {
        let all_points = self
            .G_vec
            .iter()
            .flatten()
            .chain(self.H_vec.iter().flatten());

        self.precompute = Some(Arc::new(VartimeRistrettoPrecomputation::new(all_points)));
    }
    ...
    /// Run an `optional_multiscalar_mul` operation, using the precompute table.
    ///
    /// # Inputs
    ///
    /// * `G_scalars` and `H_scalars` must have `m*n` elements.
    pub(crate) fn optional_multiscalar_mul(
        &self,
        n: usize,
        m: usize,
        G_scalars: impl Iterator<Item = Scalar>,
        H_scalars: impl Iterator<Item = Scalar>,
        dynamic_scalars: impl Iterator<Item = Scalar>,
        dynamic_points: impl Iterator<Item = Option<RistrettoPoint>>,
    ) -> Option<RistrettoPoint> {
        if let Some(precompute) = &self.precompute {
            // Pad unused points with zeros.
            let padding = (self.gens_capacity * self.party_capacity).checked_sub(m * n)?;
            // TODO: doesnt work
            let g_scalars = G_scalars.chain(iter::repeat(Scalar::ZERO).take(padding));
            let h_scalars = H_scalars.chain(iter::repeat(Scalar::ZERO).take(padding));

            precompute.optional_mixed_multiscalar_mul(
                g_scalars.chain(h_scalars),
                dynamic_scalars,
                dynamic_points,
            )
        } else {
            // No precompute tables
            let scalars = G_scalars.chain(H_scalars).chain(dynamic_scalars);
            let points = self
                .G(n, m)
                .cloned()
                .map(Some)
                .chain(self.H(n, m).cloned().map(Some))
                .chain(dynamic_points);
            RistrettoPoint::optional_multiscalar_mul(scalars, points)
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn precompute_should_not_affect_multiscalar_mul() {
        let mut rng = rand::thread_rng();

        let gens = BulletproofGens::new(64, 8);
        let mut gens_p = BulletproofGens::new(64, 8);
        gens_p.precompute_tables();

        let n = 64;
        let m = 3;

        let dyn_scalars = iter::from_fn(|| Some(Scalar::from(rng.gen::<u64>())))
            .take(19)
            .collect::<Vec<_>>();
        let dyn_points = iter::from_fn(|| Some(RistrettoPoint::random(&mut rng)))
            .map(Some)
            .take(19)
            .collect::<Vec<_>>();

        let G_scalars = iter::from_fn(|| Some(Scalar::from(rng.gen::<u64>())))
            .take(m * n)
            .collect::<Vec<_>>();
        let H_scalars = iter::from_fn(|| Some(Scalar::from(rng.gen::<u64>())))
            .take(m * n)
            .collect::<Vec<_>>();

        assert_eq!(
            gens.optional_multiscalar_mul(
                n,
                m,
                G_scalars.iter().cloned(),
                H_scalars.iter().cloned(),
                dyn_scalars.iter().cloned(),
                dyn_points.iter().cloned()
            ),
            gens_p.optional_multiscalar_mul(
                n,
                m,
                G_scalars.iter().cloned(),
                H_scalars.iter().cloned(),
